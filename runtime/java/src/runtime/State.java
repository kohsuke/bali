...
public abstract class State {
     ...
     * @param recovery
     *      true if we are recovering from an error.
     */
    public abstract State endElement(AttributesSet attributes, State partialResult, StateFactory factory, boolean recovery);
    ...
    public final State endElementFast( StateFactory factory ) {
        if( cachedEndElementFast==null )
            cachedEndElementFast = endElement( AttributesSet.empty, State.emptySet, factory, false);
     ...
     * @param nameCodeMap
     */
    public abstract State recoverFromError(State partialResult, StateFactory factory, Set expectedTokens, NameCodeMap nameCodeMap);
    ...
    private static final class Empty extends State {
        ...
        public State endElement(AttributesSet attributes, State partialResult, StateFactory factory, boolean recovery) {
        ...
        public State recoverFromError(State partialResult, StateFactory factory, Set expectedTokens, NameCodeMap nameCodeMap) {
            return partialResult;
        }
    ...
    public final static class After extends State {
        ...
        public State endElement(AttributesSet attributes, State partialResult, StateFactory factory, boolean recovery) {
            if( child.isFinal() || recovery )
                return then.expandFast( attributes, partialResult, factory );
            return partialResult;
        }
...
//            
        ...
        public State recoverFromError(State partialResult, StateFactory factory, Set expectedTokens, NameCodeMap nameCodeMap) {
            if( child.isFinal() )
                expectedTokens.add("end tag");
            State s = child.recoverFromError(emptySet,factory,expectedTokens, nameCodeMap);
            State r = factory.makeAfter( s, then );
            return factory.makeChoice( partialResult, r );
        }
    ...
    public static final class Choice extends State {
        ...
        public State endElement(AttributesSet attributes, State partialResult, StateFactory factory, boolean recovery) {
            State s1 = lhs.endElement( attributes, partialResult, factory, recovery);
            State r  = rhs.endElement( attributes, s1, factory, recovery);
        ...
        public State recoverFromError(State partialResult, StateFactory factory, Set expectedTokens, NameCodeMap nameCodeMap) {
            State s = lhs.recoverFromError( partialResult, factory, expectedTokens, nameCodeMap);
            s = rhs.recoverFromError( s, factory, expectedTokens, nameCodeMap);
            return s;
        }
    ...
    public static final class Interleave extends State {
        ...
        public State expand( AttributesSet attributes, State partialResult, StateFactory factory ) {
            ...
            if( canJoin() )
                x2 = alphabet.join.expandFast( attributes, x2, factory );
            
            return x2;
        }

        public boolean isExpandable() {
            return lhs.isExpandable() || rhs.isExpandable() || canJoin();
        }
...
//                result = alphabet.join.expandFast( attributes, result, factory );
    ...
    public static final class Single extends State {
        ...
        public State recoverFromError(State result, StateFactory factory, Set expectedTokens, NameCodeMap nameCodeMap) {
            if(dTr!=null || lTr!=null || vTr!=null)
                expectedTokens.add("text"); // TODO: figure out what text is expected

            for( Transition.Data da=dTr; da!=null; da=da.next )
                result = factory.makeChoice(result,da.right);

            for( Transition.Value va=vTr; va!=null; va=va.next )
                result = factory.makeChoice(result,va.right);

            for( Transition.List la=lTr; la!=null; la=la.next )
                result = factory.makeChoice(result,la.right);

            for( Transition.Element ea=eTr; ea!=null; ea=ea.next )
                result = recoverFromError(nameCodeMap, ea, expectedTokens, factory, result);

            for( int i=0; i<quickETr.length; i++ )
                // no need to look at nexts because they all expect the same elements
                result = recoverFromError(nameCodeMap, quickETr[i], expectedTokens, factory, result);

            return result;
        }

        private State recoverFromError(NameCodeMap nameCodeMap, Transition.Element ea, Set expectedTokens, StateFactory factory, State result) {
            nameCodeMap.find(ea,expectedTokens);
            result = factory.makeChoice(result,ea.right);
            return result;
        }
