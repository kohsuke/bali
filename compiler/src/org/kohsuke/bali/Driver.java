...
public class Driver {
    ...
    private static int usage( String msg ) {
        ...
        System.err.println(
            ...
            "  -oj[opts] <target dir> <fully qualified class name> :\n"+
            "        output Java validatelet\n"+
            "        opts: p : add JAXP 1.3 support to the Java validatelet\n"+
            "              v : add JARV support to the Java validatelet\n" +
            "        e.g., -ojpv, -ojv, etc.\n"+
            "  -ocs <target dir> <fully qualified class name> :\n"+
            "        output C# validatelet\n"+
            "  -ot :\n"+
            ...
            try {
                for( int i=0; i<args.length; i++ ) {
                    ...
                    if("-/".indexOf(arg.charAt(0))!=-1) {
                        ...
                        if(opt.startsWith("oj")) {

                            String rest = opt.substring(2);
                            boolean jaxp = rest.indexOf('p')>=0;
                            boolean jarv = rest.indexOf('v')>=0;
                            ...
                            if(idx==-1) {
                                ...
                                pkgName = fullClassName.substring(0,idx);
                                className = fullClassName.substring(idx+1);
                            }

                            writer = new MultiWriter( writer,
                                new JavaWriter( pkgName, className, target, jaxp, jarv ));
...
//            return -2;
        ...
        try {
            ...
            Validatelet v = interpreter.createValidatelet();
            v.setErrorHandler(new ErrorReporter());
            ...
            for (Iterator itr = instances.iterator(); itr.hasNext();) {
                ...
                System.out.println(v.hadError()?"invalid":"valid");
            }
        } catch( SAXException e ) {
            printException(e,verbose);
            return 0;
